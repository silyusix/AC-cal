...
import { designLagLeadCompensator } from '../services/api'; // Ensure this line is present
...
// Helper function to format transfer function polynomials to string
const formatPolynomialToString = (coeffs) => {
  if (!coeffs || coeffs.length === 0) return '0'; // Handle empty or invalid coeffs
  let s = '';
  const degree = coeffs.length - 1;
  for (let i = 0; i < coeffs.length; i++) {
    const coeff = coeffs[i];
    const power = degree - i;
    if (Math.abs(coeff) < 1e-6 && power !== 0 && coeff === 0) continue; // Skip very small coeffs if not constant term
    ...
    } else if (power === 1) { // s term
      term = `${(Math.abs(absCoeff - 1) < 1e-6) ? '' : absCoeff.toFixed(3)}s`;
    } else { // s^n term
      term = `${(Math.abs(absCoeff - 1) < 1e-6) ? '' : absCoeff.toFixed(3)}s^${power}`;
    }
  ...
  return s.trim().replace(/^ \+ /, ''); // Clean up leading "+ " if any
};
...
const LagLeadCompensation = () => {
  ...
  const [desiredPM, setDesiredPM] = useState('50'); // Default desired phase margin
  const [desiredKv, setDesiredKv] = useState('10'); // Default desired Kv
  ...
  const handleDesign = async () => {
    ...
    try {
      ...
      // Basic validation
      if (num_coeffs.length === 0 || den_coeffs.length === 0 || isNaN(pm) || isNaN(kv)) {
        throw new Error('请输入有效的传递函数系数、期望相角裕度和期望Kv。');
      }

      // Call the API service function
      const response = await designLagLeadCompensator({
  ...
  useEffect(() => {
    if (result?.plots && result?.performance) { // Check if result and its plots/performance properties exist
      ...
      if (stepPlotRef.current) {
        ...
        const layout = {
          title: '闭环阶跃响应对比', xaxis: { title: '时间 (s)' }, yaxis: { title: '幅值' },
          legend: { x: 0.1, y: 1.2, orientation: 'h' }, height: 300,
        };
  ...
  // --- JSX Render ---
  return (
    <Card title={<Title level={4}>超前-滞后校正设计</Title>} variant="borderless">
      <Row gutter={[16, 24]}>
        {/* Left Column: User Inputs */}
        <Col xs={24} lg={8}>
          <Title level={5}>1. 输入系统和目标</Title>
          <Paragraph>输入原始开环传递函数 G(s)H(s) 的系数、期望相角裕度以及期望的稳态速度误差系数 Kv。</Paragraph>
          ...
          <Input addonBefore="期望相角裕度 (°)" placeholder="例如: 50" value={desiredPM} onChange={e => setDesiredPM(e.target.value)} style={{ marginTop: '10px' }} />
            ...
            {result && (
              <Row gutter={[16, 16]}>
                ...
                <Col span={24}>
                  <Card type="inner" title="性能指标对比" style={{ marginBottom: '10px' }}>
                                      <Descriptions bordered column={2} size="small">
                                        {Object.entries({
                                          '原始 Kv': result.performance.before.kv,
                                          '校正后 Kv': result.performance.after.kv,
                                          ...
                                          <Descriptions.Item key={label} label={label}>
                                            {(value != null ? value.toFixed(2) : 'N/A') + (label.includes('裕度') ? ' dB' : label.includes('Kv') ? '' : label.includes('频率') ? ' rad/s' : '°')}
                                          </Descriptions.Item>
                                        ))}
                                      </Descriptions>                  </Card>
                </Col>
                ...
                {/* Compensator Details Card */}
                              <Col span={24}>
                                <Card type="inner" title="设计的超前-滞后校正装置 Gc(s)" style={{ marginBottom: '10px' }}>
                                                                                        <Descriptions bordered column={1} size="small">
                                                                                          <Descriptions.Item label="传递函数">
                                                                                            <MathJax>
                                                                                              {`$ G_c(s) = \\frac{${formatPolynomialToString(result.compensator.num)}}{${formatPolynomialToString(result.compensator.den)}} $`}
                                                                                            </MathJax>
                                                                                          </Descriptions.Item>
                                                                                        </Descriptions>                                </Card>
                              </Col>
...
export default LagLeadCompensation;
